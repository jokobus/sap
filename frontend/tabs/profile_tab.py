    ...
    try:
        if parse_all is not None:
            ...
            if asyncio.iscoroutinefunction(parse_all):
                resume_json, linkedin_json, github_json, candidate_json = asyncio.run(parse_all(resume_bytes, linkedin_bytes, github_username))
            else:
                resume_json, linkedin_json, github_json, candidate_json = parse_all(resume_bytes, linkedin_bytes, github_username)
        else:
            candidate_json = {
        ...
        st.session_state["linkedin_json"] = linkedin_json
        st.session_state["github_json"] = github_json
        #
